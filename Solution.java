...
public class Solution {
    ...
    */
    ...
    private boolean oddNeighborhood(int i, int j) {
        ...
        int total = 0;
        if(board[i][j]){    //itself is tapped
            total++;
        }
        if((i > 0) && (board[i-1][j])) {    //cell in row above is tapped
            total++;
        }
        if((i < height -1 ) && (board[i+1][j])) {   //cell in row below is tapped
            total++;
        }
        if((j > 0) && (board[i][j-1])) {    //cell in column to the left is tapped
            total++;
        }
        if((j < (width - 1)) && (board[i][j+1])) {    //cell i column to the right
            total++;
        }
        return (total%2)== 1 ;  //is ON when it's flipped odd number of times
    }
    ...
    public boolean stillPossible(boolean nextValue, GameModel model){
      ...
      if((i > 0) && (!oddNeighborGame(i-1,j,model))){
          possible = false;
      }
      if(possible && (i == (height-1))) {
          if((j > 0) && (!oddNeighborGame(i,j-1,model))){
              possible = false;
          }
          if(possible && (j == (width-1))&& (!oddNeighborGame(i,j,model))){
              possible = false;
          }
      }
      board[i][j] = before;
    ...
    public boolean finish(GameModel model){
      ...
      while(currentIndex < height*width) {
          if(i < height - 1 ) {
              setNext(!oddNeighborGame(i-1,j,model));
              i = currentIndex/width;
              j = currentIndex%width;
          } else { //last raw
              if(j == 0){
                 ...
                 if((height > 1) && oddNeighborGame(i-1,j,model) != oddNeighborGame(i,j-1,model)){
                   return false;
                 }
                 setNext(!oddNeighborGame(i,j-1,model));
              }
              i = currentIndex/width;
              j = currentIndex%width;
          }
      }
      if(!oddNeighborGame(height-1,width-1,model)){
          return false;
      }
      // here we should return true because we could
      // successfully finish the board. However, as a
    ...
    private boolean oddNeighborGame(int i, int j, GameModel g) {
        if(i < 0 || i > height - 1 || j < 0 || j > width - 1) { //impossible position
            return false;
        }

        int total = 0;
        if(board[i][j]){ // checking itself
            total++;
        }
        if((i > 0) && board[i-1][j] ) { // checking above
            total++;
        }
        if((i < height -1 ) && board[i+1][j] ) { // checking below
            total++;
        }
        if((j > 0) && board[i][j-1] ) { // checking left
            total++;
        }
        if((j < (width - 1)) && board[i][j+1] ) { // checking right
            total++;
        }

        if(g.isON(i,j)){  // checking same location in model
          total++;
        }
    ...
    public boolean get(int i, int j){
      ...
      return board[i][j]; // return true if tapped in solution
    }
}
